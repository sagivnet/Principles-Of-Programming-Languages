...
// new varDecl maker
export const newMakeVarDecl =  (sexps: string[] | string )  : VarDecl | Error =>
((sexps.length===2) && (sexps[1]==="lazy") ? makeVarDecl(sexps[0], true) :
(isString(sexps) ? makeVarDecl(sexps, false):Error('Error')));
...
const parseProcExp = (sexps: any[]): ProcExp | Error =>
    safeFL((body: CExp[]) => makeProcExp( map(newMakeVarDecl, sexps[1]), body))
        (map(parseL3CExp, rest(rest(sexps))));
        const isStringArray= (x:any): x is  String[] => (isArray(x) && allT(isString, x));

export const parseDecls = (sexps: any[]): VarDecl[] | Error =>{
    if(allT(isString, sexps)){
        let decls: (VarDecl|Error)[]= map(newMakeVarDecl, sexps);
    if(allT(isStringArray,sexps)){
        let decls: (VarDecl|Error)[]= map(newMakeVarDecl, sexps);
    }
