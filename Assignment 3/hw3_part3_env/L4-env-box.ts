...
import { Thunk, isThunk, makeThunk, force  } from './L4-eval-box';
...
export const applyEnv = (env: Env, v: string): Value4 | Error => {
    const binding = applyEnvBdg(env, v);
    const val = isError(binding) ? binding :getFBindingVal(binding);
    return isThunk(val)? force(val) : val;
}
...
export const ExtEnvVals = (env: ExtEnv): (Value4|Thunk)[] =>
