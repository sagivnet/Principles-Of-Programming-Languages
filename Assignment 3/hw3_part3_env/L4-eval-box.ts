...
export type Thunk ={tag:"thunk", arg: CExp4, env: Env}
export const makeThunk =(arg: CExp4, env:Env):Thunk => ({tag:"thunk", arg: arg, env: env});
export const isThunk = (e:any):e is Thunk => e.tag==="thunk";

export const force= (t: Thunk): Value4|Error => L4applicativeEval(t.arg, t.env);
export const forceAllThunks= (args:(Thunk|Value4)[])=> {
    const evaled_args=  map((arg:Thunk|Value4)=> isThunk(arg)? force(arg) : arg ,args);
    return hasNoError(evaled_args)? 
        evaled_args :
        Error(`Bad Arg:
                ${getErrorMessages(evaled_args)}`);
}

export const normalEvalArgs= (proc:Closure4, args: CExp4[], applicationEnv:Env): Array<Value4|Thunk> |Error=> {
    let pairs = zip(proc.params, args);
    let vals_and_thunks = map((pair):Value4|Thunk|Error=>
        first(pair).lazy? 
            makeThunk(second(pair), applicationEnv) :
            L4applicativeEval(second(pair), applicationEnv)
        ,pairs);
    return hasNoError(vals_and_thunks) ? vals_and_thunks :
        Error(`Bad args: ${getErrorMessages(vals_and_thunks)}`);
};
...
export const newApplyPrimitive = (proc: PrimOp, args: CExp4[],env:Env): Value4 | Error =>{
    const params: (Value4|Error)[]= map((arg) => L4applicativeEval(arg, env), args);
        return hasNoError(params) ? 
            applyPrimitive(proc, params):
            Error(`bad arg: ${getErrorMessages(params)}`);
}

const newApplyClosure4 = (proc: Closure4, args: CExp4[],env:Env): Value4 | Error => {
    const evalArgs: (Value4|Thunk)[] | Error= normalEvalArgs(proc, args, env);
    return isError(evalArgs)? evalArgs:
        applyClosure4(proc, evalArgs);
}
...
const L4applyProcedure = (proc: Value4 | Error, args: CExp4[], env: Env): Value4 | Error =>
    ...
    isPrimOp(proc) ? newApplyPrimitive(proc, args,env) :
    isClosure4(proc) ? newApplyClosure4(proc, args,env) :
...
const applyClosure4 = (proc: Closure4, args: (Value4|Thunk)[] ): Value4 | Error => {
    ...
    return evalExps(proc.body, makeExtEnv(vars, args, proc.env)); //Not Finished
}
...
const evalLet4 = (exp: LetExp4, env: Env): Value4 | Error => {
    const vals: Array <Value4 |Thunk| Error> =
    map((binding) => binding.var.lazy? 
        makeThunk(binding.val,env) : 
        L4applicativeEval(binding.val, env), exp.bindings);
