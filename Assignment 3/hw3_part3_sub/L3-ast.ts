...
// Q3
export interface VarDecl {tag: "VarDecl"; var: string; lazy: boolean};
...
// Q3
export const makeVarDecl = (v: string, l: boolean): VarDecl => ({tag: "VarDecl", var: v, lazy: l});
...
const parseDefine = (es: any[]): DefineExp | Error =>
    ...
    //ass3 , we didn't get a lazy in decleration, that's why it's false
...
//Q3 change in parser - if we get a lazy declration => true 
const parseProcExp = (sexps: any[]): ProcExp | Error =>
    safeFL((body: CExp[]) => makeProcExp(map(makeVarDeclOrLazyDecl, sexps[1]), body))
        (map(parseL3CExp, rest(rest(sexps))));
//Q3 - checking if we have a lazy in the declration and give the right lazy boolean
export const makeVarDeclOrLazyDecl =  (sexps: string[] | string )  : VarDecl | Error =>
    (isString(sexps) ? makeVarDecl(sexps, false) :
    ((sexps.length===2) && (sexps[1]==="lazy") ? makeVarDecl(sexps[0], true) : Error(`Expected (lambda <var> <lazy>) - ${sexps[0]}`)));


//Q3
const stringOrStringArray= (x:any): x is (String| String[]) => isString(x) || (isArray(x) && allT(isString, x))
//Q3
export const parseDecls = (sexps: any[]): VarDecl[] | Error =>{
    if(allT(stringOrStringArray, sexps)){
        let decls: (VarDecl|Error)[]= map(makeVarDeclOrLazyDecl, sexps);
        return  hasNoError(decls) ? decls:
                Error(`Error parsing Decls: ${getErrorMessages(decls)}`);
    } else
        return Error(`VarDecl must be a string - ${sexps}`);
};
