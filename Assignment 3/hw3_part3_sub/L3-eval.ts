...
import { makeAppExp, makeBoolExp, makeIfExp, makeLitExp, makeNumExp, makeProcExp, makeStrExp,
         makeVarDecl,makeVarDeclOrLazyDecl, makeVarRef } from "./L3-ast";
...
//Q3 - normal evaluation on lazy declrations 

const L3applyProcedure = (proc: Value | Error, args: CExp[], env: Env): Value | Error => {
    if (isError(proc)) {
        return proc;
    } else if (isPrimOp(proc)) {
        const argVals: Array <Value | Error> = map((arg) => L3applicativeEval(arg, env), args);
        if (hasNoError(argVals))
            return applyPrimitive(proc, argVals);
        else
            return Error(getErrorMessages(argVals));
    } else if (isClosure(proc)) {
        const varsAndVals = zip(proc.params, args);
        const semiEval= 
            map((p)=> 
                first(p).lazy?
                    second(p):
                    L3applicativeEval(second(p), env),
                    varsAndVals);

        // Sub semiEval args in the body

        const vars = map((p) => p.var, proc.params);
        const body = renameExps(proc.body);

        return hasNoError(semiEval) ?
            evalExps(substitute(body, vars, 
                map((arg)=> isCExp(arg)?        // Wrapping values into expressions
                    arg : 
                    valueToLitExp(arg)
                ,semiEval)), env):
            Error(getErrorMessages(semiEval));
    } else {
...
export const renameExps = (exps: CExp[]): CExp[] => {
    ...
    const replaceProc = (e: ProcExp): ProcExp => {
        ...
        const newArgs = map((arg:VarDecl):VarDecl=>makeVarDecl(varGen(arg.var), arg.lazy), e.args);
        const newBody = map(replace, e.body);
        return makeProcExp(newArgs,
                           substitute(newBody, oldArgs, 
                                map((arg:VarDecl):VarRef=>makeVarRef(arg.var), 
                                newArgs)));
    }
